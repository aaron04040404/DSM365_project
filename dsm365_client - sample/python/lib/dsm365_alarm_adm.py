...
# ========================================================================================================
# Important!
# All things in the class works for main admin-side services.
# ========================================================================================================
class Dsm365_Alarm_Adm (object):
    # Database connection body reference.
    def __init__ ( self, db_config ):
        # define const
        self.MYSQL_CONN_POOL_SIZE = 2
        self.DB_WRITE_BUFF_SIZE   = 5000
        self.DB_POOL_NAME         = "DSM365_TIME_WARNING_NOTIFICATION"
    ...
    def genNotificationContents ( self ):
        ...
        try:
            ...
            cursor.execute( """SELECT id FROM mcb_alarm_type WHERE id IN ( 201 ) AND level >= 2""" )
            cursor_cache = cursor.fetchall()
            ...
            Logger.logError( "MCB lost notifications error: %s." % e )
        ...
        while True:
            ...
            for ( id, alarmTitle, asset_sn, lcm_sn, displayer_id, startTime, endTime ) in rows:
                ...
                if ( urlEndTimestamp == 0 ):
                    ...
                    # When the time.mktime start to convert it, you will lost hours.It makes you start from UTC time, and then to get the UTC timestamp.
                ...
                insertManyData_content.append( (
                    ...
                    json.dumps( { "alarm_type": alarm_type, "mcb_id": displayer_id, "end_time": urlEndTimestamp, "main_sn": asset_sn } ),
                    startTime 
                ) )
            ...
            if ( len( insertManyData_content ) > 0 ):
                ...
                try:
                    ...
                    Logger.logProcess( "notification(101: %s) contents saved(%s)" % ( alarm_type, cursor.rowcount ) )
                    
                    # NOTE: 因為這邊是算給admin端，所以沒有必要區分是給那一間公司底下的用戶。
                    ...
                    # array = [ uuid, alarm type id, mcb id, mcb id, mcb id ]
                    cursor.executemany(
                               ...
                               WHERE bb.mem_id IS NULL AND 
                                     m.id IN (
                                         ...
                                         WHERE ( r.displayer > 0 OR r.displayer_warn > 0 ) AND 
                                               ( t.type = 1 OR 
                                                   t.id IN (
                                                       SELECT mem_id FROM displayer_situation_mem
                                                       WHERE company_id = 1 AND life_code = (
                                                           SELECT situation
                                                           FROM displayer
                    ...
                    Logger.logProcess( "notification(101: %s) are broadcast(%s)" % ( alarm_type, cursor.rowcount ) )
